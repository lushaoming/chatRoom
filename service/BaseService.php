...
class BaseService
{
     ...
     * @return array  数据
     */
	public static function get_detail($table,$data,$query_all = '0',$order='',$limit='')
	{
		$order_by = "";
		if($order){
		...
		$sql = " select * from ".$table." where ".$where." ".$order_by.$limit_by;
		if($query_all){
     ...
     * @return array  数据
     */
	public static function get_detail_by_where_sql($table,$where_sql,$query_all = '0',$order='',$limit='')
	{
		$order_by = "";
		if($order){
			$order_by = " order by ".$order;
		...
		$sql = " select * from ".$table." where ".$where_sql." ".$order_by.$limit_by;
		$data = array();
		if($query_all){
