...
class MCore
{
    ...
    public static $IMGS_SAVE_PATH = '../../../upload';//上传文件的保存路径
    public static $FILES_SAVE_PATH = '../../../files';
    public static $news_img_path = '../../../news_img';
    public static $prefix_path = '/scau_info';//上传文件保存地址前缀
    public static $code_path = 'chat/code/';

    public static $key = 'Lu!@admin001';  //加密key值
	...
	public static function get_var($key,$check_empty='1')
    {
        if($check_empty && ( !isset($_REQUEST[$key]) || !$_REQUEST[$key] ) ){
            MCore::ajax_return(array('code'=>'-30001','msg'=>$key.'参数不能为空'));
        }
        $ret = '';
     ...
     * @return 请求值
     */
    public static function get_not_empty_var($key,$msg='参数不能为空')
    {
        if( !isset($_REQUEST[$key]) || !$_REQUEST[$key]){
            MCore::ajax_return(array('code'=>'-30001','msg'=>$msg));
        }
        $ret = '';
        if(isset($_REQUEST[$key])){
            $ret = addslashes($_REQUEST[$key]);
    ...
    public static function get_full_url($url)
    {
        return \Yii::$app->params['full_domain'].$url;
    }
    ...
    public static function cache_shutdown_error() {  
